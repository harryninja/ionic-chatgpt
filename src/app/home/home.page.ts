...
export class HomePage {
  ...
  async saveToStorage(response: string, prompt: string) {

    await this.storage.set(prompt, prompt);
    await this.storage.set(response, response);
  }
  ...
  submit() {
    if (this.form.valid) {
      ...
      this.openAi.sendQuestion(prompt).subscribe({
        next: (res: any) => {
          this.loading = false;
          this.saveToStorage(prompt, res.choices[0].message.content);
